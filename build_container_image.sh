...
if [[ "$source_type" == "def" ]]; then
  cmd="apptainer build --fix-perms --fakeroot ${APPTAINER_ARGS[@]} $TARGET_CONTAINER $source_file"
...
if [[ "$source_type" == "Dockerfile" || "$source_type" == "image" ]]; then
  ...
  cmd3="apptainer build --fix-perms --fakeroot ${APPTAINER_ARGS[@]} $TARGET_CONTAINER oci-archive://$tmp_tarball" 
  cmd4="rm $tmp_tarball"
